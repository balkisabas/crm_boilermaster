...
<!-- end page title -->
<form action="{{ route('updateUsers', $user->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-12">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    @foreach ($branches as $k)
                                        <option value="{{$k->description}}" {{$k->description ==  $user->branch ? 'selected' : ''}}>{{$k->description}}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <option value="">-Please Select Branch-</option>
                                    @foreach ($activation_status as $m)
                                        <option value="{{$m->description}}" {{$m->description ==  $user->activation_status ? 'selected' : ''}}>{{$m->description}}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
