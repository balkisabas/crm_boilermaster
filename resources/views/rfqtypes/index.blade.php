...
@section('title', 'List of RFQ Type') 
@section('css')
<!-- DataTables -->
        ...
        <div class="card">
            <div class="card-body">
                ...
                <h3 class="card-title mb-4 d-flex justify-content-between align-items-center">List of RFQ Type
                    <a href="{{route('rfqtypes.create')}}" class="btn btn-primary"> <i class="bx bx-plus"></i> Create</a>
                </h3>
                ...
                <table id="datatable" class="table table-striped dataTable no-footer nowrap w-100">
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            @else
                            @foreach ($rfqtypes as $m)
                            <td>{{$i++}}</td>
                            <td>{{ $m->name}}</td>
                            <td>
                            <a href="{{ route('rfqtypes.show',$m->id) }}"><i class="bx bx-search-alt" title="View documents" style="color:black"></i></a>
                                @can('edit-docType')
                                    <a href="{{ route('rfqtypes.edit',$m->id) }}"><i class="bx bx-edit-alt" title="Edit/Update documents"></i></a>
                                @endcan
                                @can('delete-docType')
                                    {!! Form::open(['method' => 'DELETE','route' => ['rfqtypes.destroy', $m->id],'style'=>'display:inline']) !!}
