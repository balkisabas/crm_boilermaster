        ...
        <div class="card">
            <div class="card-body">
                <h3 class="card-title mb-4 d-flex justify-content-between align-items-center">List of Role
                    @can('create-role')
                        <a href="{{ route('roles.create') }}" class="btn btn-primary"> <i class="bx bx-plus"></i> Create</a>
                    @endcan
                </h3>
                <table id="datatable" class="table table-striped dataTable no-footer nowrap w-100">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $role->name }}</td>
                            <td>
                                <a href="{{ route('roles.show',$role->id) }}"><i class="bx bx-search-alt" title="View Details Roles" style="color:black"></i></a>
                                @can('edit-role')
                                    <a href="{{ route('roles.edit',$role->id) }}"><i class="bx bx-edit-alt" title="Edit/Update Roles"></i></a>
                                @endcan
                                @can('delete-role')
