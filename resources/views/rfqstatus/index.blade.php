        ...
        <div class="card">
            <div class="card-body">
                ...
                <h3 class="card-title mb-4 d-flex justify-content-between align-items-center">List of  RFQ Status
                    <a href="{{route('rfqstatus.create')}}" class="btn btn-primary"> <i class="bx bx-plus"></i> Create</a>
                </h3>
                ...
                <table id="datatable" class="table table-striped dataTable no-footer nowrap w-100">
                    ...
                    <tbody>
                        ...
                        @php
                            $i = 1;
                            @endphp
                            @if(count($rfqstatus) === 0)
                            <td colspan="10" align="center">No records found.</td>  
                            @else
                            @foreach ($rfqstatus as $m)
                            <td>{{$i++}}</td>
                            <td>{{ $m->name}}</td>
                            <td>
                                <a href="{{ route('rfqstatus.show',$m->id) }}"><i class="bx bx-search-alt" title="View Details RFQ Status" style="color:black"></i></a>
                                @can('edit-proposalstatus')
                                    <a href="{{ route('rfqstatus.edit',$m->id) }}"><i class="bx bx-edit-alt" title="Edit/Update RFQ Status"></i></a>
                                @endcan
                                @can('delete-proposalstatus')
                                    {!! Form::open(['method' => 'DELETE','route' => ['rfqstatus.destroy', $m->id],'style'=>'display:inline']) !!}
